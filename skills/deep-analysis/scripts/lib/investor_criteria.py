...
# ═══════════════════════════════════════════════════════════════
# NAME_KO — 규칙 이름 한국어 매핑 (v2.16.0)
# 키: (investor_id, rule_id) → 한국어 rule name
# · 기존 중국어 Rule.name 은 절대 수정하지 않음 (D8 정책)
# · evaluator 에서 get_language()=="ko" 일 때 이 dict 를 먼저 조회하고,
#   없으면 기존 중국어 Rule.name 으로 graceful fallback
# ═══════════════════════════════════════════════════════════════
NAME_KO: dict[tuple[str, str], str] = {
    # ── A 그룹 · 고전 가치파 ──────────────────────────────────
    ("buffett", "roe_5y_15"):        "ROE 5년 연속 > 15%",
    ("buffett", "net_margin_15"):    "순이익률 > 15%",
    ("buffett", "debt_ratio_50"):    "부채비율 < 50%",
    ("buffett", "fcf_positive"):     "잉여현금흐름(FCF) 양(+)의 값",
    ("buffett", "moat_clear"):       "경제적 해자 명확 (총점 24/40 이상)",
    ("buffett", "safety_margin_pe"): "PER 5년 중간값 이하",
    ("buffett", "dividend_history"): "5년 연속 배당",
    ("graham", "pe_under_15"):       "PER < 15",
    ("graham", "pb_under_1_5"):      "PBR < 1.5",
    ("graham", "pe_pb_22_5"):        "PER x PBR < 22.5 (그레이엄 22.5 법칙)",
    ("graham", "current_ratio_2"):   "유동비율 > 2",
    ("graham", "profit_10y"):        "10년 연속 흑자",
    ("graham", "dividend_history"):  "연속 배당",
    ("fisher", "industry_growing"):  "업종 시장 잠재력 있음",
    ("fisher", "profitability"):     "수익성 우수",
    ("fisher", "moat_quality"):      "품질 장벽 (해자 평균 6점 이상)",
    ("fisher", "sell_side_confirm"): "증권사 리서치 대다수 매수 추천",
    ("fisher", "growth_sustainable"):"매출 3년 CAGR > 15%",
    ("munger", "simple_business"):   "비즈니스 모델 단순·이해 가능 (주요 사업 3개 이하)",
    ("munger", "moat_strong"):       "경제적 해자 강함 (총점 28 이상)",
    ("munger", "financial_strength"):"재무 건전 (저부채 + 정(+)현금흐름)",
    ("munger", "wait_for_price"):    "밸류에이션 안전마진 있음 (PER 분위 < 40)",
    ("munger", "psych_no_mania"):    "시장 심리 과열 아님 (안전 판정)",
    ("templeton", "pe_extreme_low"): "PER 5년 저점 (분위 < 25)",
    ("templeton", "vs_industry_cheap"): "동종업 대비 저평가",
    ("templeton", "not_crowded"):    "시장 심리 과열 아님",
    ("klarman", "margin_of_safety"): "내재가치 대비 할인율 > 30%",
    ("klarman", "downside_protected"): "저부채 + 정(+)현금흐름",
    ("klarman", "catalyst_clear"):   "명확한 촉매제 존재",
    # ── B 그룹 · 성장 투자파 ──────────────────────────────────
    ("lynch", "peg_ideal"):          "PEG < 1 (Lynch 이상값)",
    ("lynch", "peg_acceptable"):     "PEG 1-1.5 (경계 수용)",
    ("lynch", "pe_not_rolls_royce"): "PER < 40 (Lynch 경계선)",
    ("lynch", "fast_grower_zone"):   "매출 증가율 20-50% (Lynch fast grower 최적 구간)",
    ("lynch", "understandable"):     "업종 이해하기 쉬움",
    ("lynch", "research_support"):   "리서치 커버리지 중립 이상",
    ("oneill", "c_eps_growth"):      "C: 분기 EPS YoY > 25%",
    ("oneill", "a_annual_growth"):   "A: 연간 EPS 성장 > 20%",
    ("oneill", "n_near_high"):       "N: 52주 신고가 근접",
    ("oneill", "l_industry_leader"): "L: 업종 선두 (성장기)",
    ("oneill", "i_institutional"):   "I: 기관 보유 증가 (펀드매니저 보유)",
    ("oneill", "m_market_trend"):    "M: 개별 종목 추세 상승 (Stage 2)",
    ("thiel", "monopoly_leader"):    "업종 독점 지위 (해자 28 이상)",
    ("thiel", "network_effect"):     "네트워크 효과 7점 이상",
    ("thiel", "scale_advantage"):    "규모 우위 7점 이상",
    ("wood", "s_curve"):             "업종 S커브 변곡점 (TAM > 20%/년)",
    ("wood", "innovation_platform"): "ARK 파괴적 혁신 플랫폼 해당",
    ("wood", "revenue_acceleration"):"매출 가속 또는 고성장",
    ("wood", "long_term_view"):      "장기 시각 (5년 후 게임 체인저)",
    ("andreessen", "software_or_ai_native"): "소프트웨어 / AI 네이티브 기업",
    ("andreessen", "rev_growth_30"): "매출 성장 > 30%",
    ("andreessen", "network_effects"):"네트워크 효과 / 플랫폼 점성",
    ("andreessen", "market_size_huge"): "TAM 1000억 달러 이상",
    ("andreessen", "founder_led"):   "창업자 여전히 주도",
    ("gurley", "marketplace_or_saas"): "marketplace / SaaS / B2B 플랫폼",
    ("gurley", "unit_economics_positive"): "단위 경제 양(+)",
    ("gurley", "magnitude_of_demand"): "수요 강도 충분",
    ("gurley", "burn_multiple_ok"):  "소각 배수(Burn Multiple) 합리적",
    ("gurley", "valuation_reasonable"): "밸류에이션 펀더멘털 이탈 아님",
    ("naval", "permissionless_leverage"): "복제 가능한 레버리지 (코드/미디어/자본)",
    ("naval", "specific_knowledge"): "독특한 지식 / 가르칠 수 없는 역량",
    ("naval", "long_holding_horizon"): "10년 이상 장기보유 적합",
    ("naval", "not_zero_sum"):       "정합 게임",
    ("naval", "compound_interest"):  "복리 곡선 명확",
    ("gerstner", "ai_or_cloud_native"): "AI / 클라우드 네이티브 / 컴퓨팅 체인",
    ("gerstner", "revenue_acceleration"): "매출 가속 (균속 아님)",
    ("gerstner", "rule_of_40"):      "Rule of 40",
    ("gerstner", "category_leader"): "업종 상위 3위 이내",
    ("gerstner", "expensive_but_growing"): "고평가이지만 성장이 뒷받침",
    ("chamath", "disruptor_thesis"): "파괴적 기업 thesis 명확",
    ("chamath", "tam_centibillion"): "TAM 1000억 달러 이상",
    ("chamath", "path_to_profit"):   "수익화 경로 명확",
    ("chamath", "not_a_meme"):       "순수 내러티브 meme 아님",
    ("chamath", "transparent_metrics"): "공시 투명",
    # ── C 그룹 · 거시 헤지파 ──────────────────────────────────
    ("soros", "sentiment_long_reflex"): "리서치 목표가 현재가 대비 > 10% 높음 (매수 반사성 기회)",
    ("soros", "sentiment_short_reflex_penalty"): "리서치 목표가 현재가 대비 > 15% 낮음 (공매도 반사성 · 매수측 감점)",
    ("soros", "macro_tailwind"):     "거시 환경 우호적",
    ("soros", "trend_clear"):        "Stage 2 추세 확인",
    ("dalio", "rate_cycle_pos"):     "금리 사이클 우호적 (금리 인하기)",
    ("dalio", "low_debt"):           "기업 부채비율 낮음",
    ("dalio", "dividend_income"):    "배당 존재 (전천후 포트 배분)",
    ("marks", "market_fear"):        "시장 탐욕 없음 (심리 < 60)",
    ("marks", "cheap_vs_history"):   "역사적 밸류에이션 저렴",
    ("marks", "risk_priced_in"):     "리스크 이미 반영됨 (낙폭 경험)",
    ("druck", "liquidity_tailwind"): "유동성 변곡점",
    ("druck", "macro_theme"):        "거시 테마 명확",
    ("druck", "high_conviction"):    "12-18개월 후 수익 가시성 (리서치 컨센서스)",
    ("robertson", "best_in_class"):  "동종업 최강 (상위 2위 이내)",
    ("robertson", "fundamentals_strong"): "펀더멘털 강함",
    ("burry", "not_in_bubble_basket"): "밸류에이션 버블 바구니 아님",
    ("burry", "insider_not_selling"): "임원 대규모 매도 없음",
    ("burry", "debt_not_explosive"): "부채 통제 가능",
    ("burry", "not_retail_mania"):   "개인 투자자 광기 없음",
    ("burry", "fcf_real_not_eps"):   "FCF 실질 · 회계 이익 아님",
    ("chanos", "not_promotional_ceo"): "CEO 과도 홍보 없음",
    ("chanos", "audited_clean"):     "감사 적정의견",
    ("chanos", "cash_matches_eps"):  "현금흐름과 이익 일치",
    ("chanos", "not_china_concept"): "중국계 미국 상장주 아님",
    ("chanos", "debt_disclosure_clean"): "부채 공시 명확",
    # ── D 그룹 · 기술 추세파 ──────────────────────────────────
    ("livermore", "stage_2"):        "Stage 2 상승기",
    ("livermore", "ma_bull"):        "이동평균 상승 배열",
    ("livermore", "volume_confirm"): "고점 근방이나 과매수 아님",
    ("minervini", "stage_2_only"):   "엄격한 Stage 2 (SEPA 핵심)",
    ("minervini", "ma_stack"):       "MA 상승 스택",
    ("minervini", "near_high"):      "52주 고점 대비 < 25%",
    ("minervini", "ytd_strong"):     "YTD 상대 강도 > 0",
    ("minervini", "not_overbought"): "RSI 심각한 과매수 아님 (< 80)",
    ("darvas", "box_breakout"):      "추세 상승 (Stage 2)",
    ("darvas", "ma_support"):        "이동평균 지지",
    ("gann", "trend_up"):            "추세 상승",
    ("gann", "volatility_normal"):   "변동성 정상 (< 60%)",
    # ── E 그룹 · 중국 가치투자파 ─────────────────────────────
    ("duan", "good_business"):       "사업 적합 (순이익률 > 15% + ROE > 10%)",
    ("duan", "good_people"):         "사람 적합 (담보 없음 + 위반 없음)",
    ("duan", "good_price"):          "가격 적합 (PER 분위 < 50)",
    ("duan", "pe_not_expensive"):    "PER 40 이하 (단융핑 가격 상한)",
    ("duan", "long_term_clear"):     "10년 이해 가능 (해자 + 성숙 사업)",
    ("zhangkun", "roe_persistent"):  "ROE 지속적 > 15% (5년 중 3회 이상)",
    ("zhangkun", "pricing_power"):   "가격결정력 (순이익률 > 18%)",
    ("zhangkun", "moat_brand"):      "브랜드/무형자산 장벽",
    ("zhangkun", "pe_discipline"):   "PER 40 이하 (장쿤 밸류에이션 기준)",
    ("zhushaoxing", "long_term_growth"): "장기 성장 (3Y CAGR > 15%)",
    ("zhushaoxing", "industry_momentum"): "업종 성장기",
    ("zhushaoxing", "low_turnover_fit"):  "저회전율 적합 (변동성 < 50%)",
    ("xiezhiyu", "garp_balance"):    "PEG 1-2 구간 (성장·밸류에이션 균형)",
    ("xiezhiyu", "growth_minimum"):  "성장률 > 10%",
    ("fengliu", "good_odds"):        "배당률(Odds) 좋음 (PER 분위 < 40 또는 큰 낙폭 경험)",
    ("fengliu", "expectation_gap"):  "기대 격차 (리서치 목표가 vs 현재가 > 15%)",
    ("fengliu", "common_sense"):     "상식 부합 (안전 + 정(+)현금흐름)",
    ("dengxiaofeng", "cycle_position"): "설비 사이클 위치 양호",
    ("dengxiaofeng", "value_creation"): "기업 가치 창출 (ROIC > 10%)",
    ("dengxiaofeng", "pe_reasonable"):  "PER 35 이하 (덩샤오펑 좌측 스타일)",
    ("dengxiaofeng", "good_price"):  "적정 가격 (PER 분위 < 60)",
    ("zhang_lei", "long_runway_industry"): "업종 10년 이상 성장 트랙",
    ("zhang_lei", "category_leader_moat"): "세부 선두 + 해자",
    ("zhang_lei", "founder_aligned"):    "창업자 장기주의 정렬",
    ("zhang_lei", "compounder_track_record"): "복리 실적 입증",
    ("zhang_lei", "not_just_cyclical"):  "순수 사이클 아님",
    # ── F 그룹 · A주 유동 세력파 ────────────────────────────
    **{
        (inv, "stage_2"): "Stage 2 상승"
        for inv in [
            "zhang_mz", "sun_ge", "zhao_lg", "yangjia", "chen_xq",
            "hu_jl", "fang_xx", "zuoshou", "xiao_ey", "jiao_yy",
            "mao_lb", "xiao_xian", "liuyi_zl", "gu_bl", "wang_zr",
    ...
    **{
        (inv, "top_of_sector"): "업종 선두 (상위 3위 이내)"
        for inv in [
            "zhang_mz", "sun_ge", "zhao_lg", "chen_xq",
            "zuoshou", "jiao_yy", "xiao_xian", "liuyi_zl", "gu_bl",
        ]
    ...
    **{
    ...
    ("xiao_ey", "fundamentals_ok"):  "펀더멘털 보조 선정 (ROE > 10%)",
    # ghzw (고해적왕 · 타오구바 10년 실전)
    ("ghzw", "mainline_theme"):      "주선 테마 확인 (섹터 열기 + 촉매)",
    ("ghzw", "limit_up_gene"):       "상한가 DNA / 용호방 활성",
    ("ghzw", "strong_tape"):         "시세판 강도 (Stage 2 + 거래량 뒷받침)",
    ("ghzw", "low_position_logic"):  "논리 확실한 저점 종목",
    ("ghzw", "era_carrier"):         "시대 감성 담체 (격국주 판정)",
    ("ghzw", "liquidity_exit"):      "유동성 진출입 가능 (거래 활성)",
    # ── G 그룹 · 퀀트 시스템파 ──────────────────────────────
    ("simons", "statistical_edge"):  "통계적 우위 (최근 1년 양(+)수익률)",
    ("simons", "volatility_tradeable"): "변동성 매매 가능",
    ("thorp", "positive_ev"):        "양(+)의 기대값 (리서치 목표 프리미엄 > 10%)",
    ("thorp", "kelly_ok"):           "켈리 공식 포지션 실행 가능 (변동성 < 50%)",
    ("shaw", "quality_factor"):      "품질 팩터 (ROE > 12%)",
    ("shaw", "value_factor"):        "가치 팩터 (PER 분위 < 60)",
    ("shaw", "momentum_factor"):     "모멘텀 팩터 (Stage 2)",
    ("shaw", "growth_factor"):       "성장 팩터 (3Y CAGR > 15%)",
    ("asness", "value_factor"):      "가치 팩터: PER / PBR 동시 저점",
    ("asness", "quality_factor"):    "품질 팩터: 높은 ROE + 저레버리지",
    ("asness", "momentum_factor"):   "모멘텀 팩터: 6-12개월 시장 초과 수익",
    ("asness", "profitability_consistent"): "수익성 일관성",
    ("asness", "not_lottery_ticket"): "복권주 아님",
    # ── I 그룹 · AI 핵심 병목 ────────────────────────────────
    ("serenity", "ai_chain_hit"):    "제품이 AI 산업 체인 핵심 고리",
    ("serenity", "chokepoint_strong"): "병목 평가 강함 (chokepoint 70 이상)",
    ("serenity", "irreplaceable"):   "상류 대체 불가 (전환비용 + 규모 장벽 높음)",
    ("serenity", "smallcap_elastic"): "중소 시총 고탄력성 (시총 < 3000억)",
    ("serenity", "demand_inflection"): "수요 변곡점 신호 (정책/촉매/업종 고성장)",
    # ── H 그룹 · AI 기술 리더파 ──────────────────────────────
    ("jensen_huang", "ai_compute_demand"):           "AI 컴퓨팅 수요 강한 상관관계",
    ("jensen_huang", "cuda_ecosystem_proxy"):        "CUDA / 생태계 결합",
    ("jensen_huang", "data_center_capex_beneficiary"): "데이터센터 자본 지출 수혜",
    ("jensen_huang", "gross_margin_strong"):         "매출총이익률 50% 이상",
    ("jensen_huang", "light_speed_moore_compliant"): "광속 무어의 법칙 속도 부합",
    ("musk", "first_principles_industry"): "제1원리로 파괴 가능한 업종",
    ("musk", "vertical_integration"):     "수직 계열화 수준 높음",
    ("musk", "manufacturing_scale"):      "제조 규모 / 양산",
    ("musk", "not_legacy_oem"):           "전통 OEM 아님",
    ("musk", "ceo_visible"):              "경영진 공개적으로 thesis 제시",
    ("altman", "agi_supply_chain"):       "AGI 상·하류 (컴퓨팅/데이터/에너지/응용)",
    ("altman", "scaling_laws_compliant"): "스케일링 법칙 수혜 부합",
    ("altman", "platform_or_infra"):      "플랫폼 / 인프라형",
    ("altman", "energy_or_compute_bottleneck"): "에너지 / 컴퓨팅 병목 카드",
    ("altman", "not_pure_consumer_app"):  "순수 소비자 앱 아님",
    ("saylor", "btc_or_digital_asset_exposure"): "BTC / 디지털 자산 노출",
    ("saylor", "treasury_strategy_signal"):      "재무 전략 / 인플레이션 헤지 자산",
    ("saylor", "not_just_eps_play"):             "순수 EPS 게임 아님",
    ("saylor", "hard_money_thesis"):             "경화(Hard Money) 내러티브 부합",
    ("saylor", "fiat_devaluation_beneficiary"):  "법정화폐 가치하락 수혜 방향",
}


def get_ko_name(investor_id: str, rule_id: str) -> str:
    """규칙 이름 한국어 반환. 없으면 '' 반환."""
    return NAME_KO.get((investor_id, rule_id), "")
